...
import { TELEGRAM_TOKEN, HARD_CAP_ETH, LAPTOP, POOLS, countdown, type SniperWallet } from './config.js';
import { http_, erc20Abi } from './chain.js';
import { getOrCreate, get, setSpend, all, armedWallets, privateKeyOf } from './store.js';
...
const OWNER_HELP = [
  '',
  '<b>Owner</b>',
  '/prep — wrap and approve everyone. Do it before 1pm.',
...
async function handle(msg: TgMessage): Promise<void> {
  ...
  if (cmd === '/start' || cmd === '/help') {
    ...
    const bal = await ethBalance(w.address);
    const funded = bal > 0n;
  ...
  if (cmd === '/balance') {
    ...
    await send(
      chatId,
      `<code>${w.address}</code>\n` +
        `ETH <b>${formatEther(bal)}</b>\n` +
        (w.spendEth ? `set to spend <b>${w.spendEth}</b> ETH` : 'no spend amount set yet — /spend 0.02'),
    );
    return;
  }
  ...
  if (cmd === '/spend') {
    ...
    const bal = await ethBalance(w.address);
    if (bal < parseEther(String(amount))) {
      return void send(
        chatId,
        `Set to ${amount} ETH, but the wallet only holds ${formatEther(bal)}.\n` +
          `Fund <code>${w.address}</code> on Base before 1pm or it will not fire.`,
      );
    }
    setSpend(from.id, amount);
    await send(chatId, `✅ You are in for <b>${amount} ETH</b>.\nWallet ${formatEther(bal)} ETH. Nothing else to do.`);
    return;
  }
